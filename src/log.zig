...
pub fn setLogLevel(loglvl: LogLevel) void {
    loglevel = loglvl;
}
...
pub const LogType = enum {
    ...
    pub fn getOutStream(comptime self: Self) std.fs.File {
        return switch (self) {
            .fatal, .err, .warn, .ok, .info => std.io.getStdErr(),
        };
    }
    ...
    fn colored(comptime self: Self, comptime verbose: bool, msg: []const u8) []const u8 {
        ...
        const prefix = comptime switch (self) {
            .fatal => style ++ RED ++ "[smed fatal]: ",
            .err => style ++ RED ++ "[smed err]: ",
            .warn => style ++ YELLOW ++ "[smed warning]: ",
            .ok => style ++ GREEN ++ "[smed ok]: ",
            .info => "[smed info]: ",
        };
